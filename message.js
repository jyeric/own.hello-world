...
if(!norunFlag){
	...
	(function (){
		...
		if(document.referrer !== ''){
			...
			}else if(document.referrer.indexOf(Live2dHistoireHomePath) !== -1){
				text = '嗨！ 你好呀！';
			}
		}else {
			if (window.location.href == Live2dHistoireHomePath) { //主页URL判断，需要斜杠结尾
	...
	function showHitokoto(){
		if(sessionStorage.getItem("Sleepy")!=="1"){
			if(!AITalkFlag){
				...
				for(var i=0;i<live2d_weiyu_cache.length;i++){ var histoiremsgimg ; if(live2d_weiyu_cache[i].img!="=''){" +histoiresite+live2d_weiyu_cache[i].img.replace('thum-', '')+'" target="_blank"><img src="'+histoireSite+live2d_weiyu_cache[i].img.replace('thum-', '')+'">'
					}
...
</=$('input[name=live2dbgm]').length-1){></live2d_weiyu_cache.length;i++){></=></=></=></=></=></=></=></=></norunai.length;i++){>